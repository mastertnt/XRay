...
namespace XSystem
{
    ...
    public static class TypeExtensions
    {
        ...
        public static IEnumerable<Type> GetInheritedTypes(this Type pThis, bool pCanBeAbstract = false, int pConstructorArgCount = 0, Assembly pSourceAssembly = null)
        {
            ...
            {
                string lSourceAssemblyName = lSourceAssembly.GetName().FullName;
                List<Assembly> lAssemblyToCheck = new List<Assembly>();
                Assembly[] lAssemblies = AppDomain.CurrentDomain.GetAssemblies();
                foreach
                    ( Assembly lAssembly in lAssemblies )
                {
                    string lCurrentAssemblyName   = lAssembly.GetName().FullName;
                    if 
                        ( lCurrentAssemblyName != lSourceAssemblyName )
                    {
                        AssemblyName[] lAssemblyNames = lAssembly.GetReferencedAssemblies();
                        AssemblyName lMatch = lAssemblyNames.FirstOrDefault( pAssemblyName => pAssemblyName.FullName == lSourceAssemblyName );
                        if
                            ( lMatch != null )
                        {
                            lAssemblyToCheck.Add( lAssembly );
                        }
                    }
                }
                ...
                if (lAssemblyToCheck.Contains(lSourceAssembly) == false)
                {
                    // Do not add it twice!
                ...
                {
                    msInheritedTypesCache.Add(lKey, lAllTypes.Where(pType => pType.IsInterface == false && pType.IsAbstract == pCanBeAbstract && pThis.IsAssignableFrom(pType) && pType.GetConstructor(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, null, Type.EmptyTypes, null) != null));
                }
                else
                {
                    ...
                    IEnumerable<Type> lFilterdTypes = lFoundTypes.Where(pType => pType.GetConstructors(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Any(pConstructor => pConstructor.GetParameters().Count() == pConstructorArgCount));
